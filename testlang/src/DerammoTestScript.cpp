...
Array DerammoTestScript::_get_rpc_methods() const
{
	return Array();
}
...
class DerammoTestScriptInstance: public Object
{
	GDCLASS(DerammoTestScriptInstance, Object)
...
{
	return memnew(DerammoTestScriptInstance());
}
#endif

Variant DerammoTestScriptResourceLoader::_load(const String& p_path, const String& p_original_path, bool p_use_sub_threads, int64_t p_cache_mode) const {
...
int64_t DerammoTestScriptResourceSaver::_save(const String& path, const Ref<Resource>& resource, int64_t flags)
{
	Ref<File> out;
	out.instantiate();
